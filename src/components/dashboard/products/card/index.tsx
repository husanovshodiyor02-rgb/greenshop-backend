...
const Card: FC<ProductType> = (props) => {
  ...
  useEffect(() => {
    setIsWishlisted(!!wishlist.find((item) => item._id === props._id));
  }, [wishlist, props._id]);

  const icon_style =
    "w-[35px] h-[35px] bg-white rounded-lg flex justify-center items-center text-[#353535] border border-white shadow-md transition-all duration-300";

  const handleViewDetail = () => {
    const category = props.category || "house-plants";
    navigate(`/shop/${category}/${props._id}`);
  };
    ...
    <div className="relative transition-all duration-300">
      <div className="group hover:border-t-2 border-white hover:border-nav rounded-[9px] h-75 bg-[#f2f2f2] flex justify-center items-center relative overflow-hidden transition-all duration-300">
        ...
        <div
          ...
          `}
        >
          <div onClick={handleCart} className={icon_style}>
            <i className="ri-shopping-cart-2-line text-xl"></i>
          </div>

          <div
            onClick={handleWishlist}
            className={`${icon_style} ${isWishlisted ? "text-red-500" : ""}`}
          >
            <i className="ri-heart-line text-xl"></i>
          </div>

          <div onClick={handleViewDetail} className={icon_style}>
      ...
      <div className="mt-3">
        ...
        <div className="flex items-center gap-3">
          <h1 className="text-nav text-[18px] font-bold">${props.price}</h1>
          {props.discount && (
